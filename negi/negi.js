(function (f, a, d, h) {
    ...
    f.Negi = (function () {
        function i() {
            ...
            this.setInlineCSS(this.__instance, {
                left: Math.random() * l + 'px',
                top: '-10px'
            });
            this.__speed = 0;
        ...
        i.prototype.start = function () {
            ...
            var k = this,
                j = function (_css_rotateZ) {
                    var p, o, t, s = 0,
                        r = 0,
                        _css_scale, n;
                    k.__run_times++;
                    for (o = 0, t = k.__fibo.length; o < t; o++) {
                        p = k.__fibo[o];
                        if (k.__run_times % p === 0) {
                            k.__speed += Math.random() - 0.5
                        }
                    }
                    n = k.getInlineCSS(k.__instance);
                    if (n.hasOwnProperty('left')) {
                        s = parseFloat(n.left)
                    }
                    ...
                    r = _css_rotateZ * 0.25;
                    _css_scale = 1 + _css_rotateZ * 0.001;
                    _css_scale = _css_scale > 2 ? 2 : _css_scale;
                    var l = {};
                    l.transform = `rotateZ(${_css_rotateZ}deg) scale(${_css_scale}) translate(${s}px,${r}px)`;
